...
const tug_o_war = teams => {
  const teamOne = teams[0].reduce((acc, curr) => acc + curr);
  const teamTwo = teams[1].reduce((acc, curr) => acc + curr);
  if (teamOne > teamTwo) return "Team 1 wins!";
  if (teamOne < teamTwo) return "Team 2 wins!";
  else {
    if (teams[0][0] > teams[1][teams[1].length - 1]) return "Team 1 wins!";
    if (teams[0][0] < teams[1][teams[1].length - 1]) return "Team 2 wins!";
